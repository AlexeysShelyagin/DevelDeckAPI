...
void DD_accel::init(int sda_pin, int scl_pin){
    ...
    _i2c.beginTransmission(DD_ACCEL_I2C_ADDRESS);
	_i2c.write(REG_CMD_BMI160);
	_i2c.write(0x11);
	_i2c.endTransmission();

    invert_mask = vec3(
        (DD_ACCEL_INV_X) ? -1.0f : 1.0f, 
        (DD_ACCEL_INV_Y) ? -1.0f : 1.0f, 
        (DD_ACCEL_INV_Z) ? -1.0f : 1.0f
    );
...
void DD_accel::auto_calibrate(){
    ...
    _i2c.requestFrom(DD_ACCEL_I2C_ADDRESS, 1);
    if(_i2c.available())
        chip = _i2c.read();
}
...
void DD_accel::set_current_as_zero(bool hold_x_axis){
    vec3 basis_z;
    for(uint8_t i = 0; i < DD_ACCEL_CALIBR_MEASURE_N; i++){
        basis_z += get_accel();
        delay(1);
    }
...
vec3 DD_accel::get_accel(){
    ...
    if(chip == BMI160_ID){
        ...
        _i2c.requestFrom(DD_ACCEL_I2C_ADDRESS, 6);
    ...
    vec3 data(ax, ay, az);
    data *= (GRAVITY_MS2 / DD_ACCEL_SENS);
    data *= invert_mask;
